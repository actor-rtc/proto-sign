...
fn test_message_no_delete_basic() {
    ...
    assert_eq!(
        ...
        3,
        "Should detect 3 message deletions, found: {:?}",
        message_delete_changes
    );
    ...
    assert!(deleted_messages.contains(&"Two"), "Should detect deletion of message 'Two'");
    assert!(deleted_messages.contains(&"Three.Four.Five"), "Should detect deletion of nested message 'Three.Four.Five'");
    assert!(deleted_messages.contains(&"Three.Seven"), "Should detect deletion of nested message 'Three.Seven'");
}
...
fn test_message_no_delete_no_changes() {
    ...
    assert!(!result.has_breaking_changes, "Identical files should not have breaking changes");
...
fn test_breaking_config_rule_exclusion() {
    ...
    assert_eq!(message_changes.len(), 0, "MESSAGE_NO_DELETE should be excluded");
}
...
fn test_breaking_result_summary() {
    ...
    assert!(result.summary.contains_key("FILE"), "Summary should contain FILE category");
