...
fn test_extension_no_delete_rule_works() {
    ...
    assert!(
        ...
        "EXTENSION_NO_DELETE规则应该检测到extension被删除"
    );
...
fn test_package_extension_no_delete_rule_works() {
    ...
    assert!(
        !result.changes.is_empty(),
        "PACKAGE_EXTENSION_NO_DELETE规则应该检测到extension被删除"
    );
...
fn test_file_same_java_generic_services_rule_works() {
    ...
    assert!(
        !result.changes.is_empty(),
        "FILE_SAME_JAVA_GENERIC_SERVICES规则应该检测到选项变化"
    );
...
fn test_file_same_cc_enable_arenas_rule_works() {
    ...
    assert!(
        !result.changes.is_empty(),
        "FILE_SAME_CC_ENABLE_ARENAS规则应该检测到选项变化"
    );
...
fn test_message_same_json_format_rule_works() {
    ...
    assert!(
        !result.changes.is_empty(),
        "MESSAGE_SAME_JSON_FORMAT规则应该检测到格式变化"
    );
    assert_eq!(result.changes[0].rule_id, "MESSAGE_SAME_JSON_FORMAT");
    assert!(
        result.changes[0]
            .message
            .contains("message_set_wire_format")
    );
}
...
fn test_enum_same_json_format_rule_works() {
    ...
    previous_enum
        .options
        .insert("json_format".to_string(), "ALLOW".to_string());
    ...
    current_enum
        .options
        .insert("json_format".to_string(), "DISALLOW".to_string()); // 也改变了
...
fn test_all_previously_fake_rules_are_implemented() {
    ...
    for rule in previously_fake_rules {
        ...
        // 应该能运行规则而不会出错
        let result = engine.check(&file1, &file2, &config);
        assert_eq!(result.changes.len(), 0, "规则 {rule} 应该被正确实现和注册");
    }
}
