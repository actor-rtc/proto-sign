...
use crate::canonical::{CanonicalFile, CanonicalMessage, CanonicalEnum, CanonicalService};
    ...
    if !prev_package.is_empty() && prev_package != curr_package {
        ...
        if curr_package.is_empty() {
            changes.push(create_breaking_change(
                "PACKAGE_NO_DELETE",
                format!(
                    "Package \"{}\" was deleted (file no longer declares package).",
                    prev_package
                ),
        ...
        for (enum_path, _prev_enum) in &prev_enums {
            if !curr_enums.contains_key(enum_path) {
                changes.push(create_breaking_change(
                    ...
                    format!(
                        "Enum \"{}\" was deleted from package \"{}\".",
                        enum_path, prev_package
                    ),
        ...
        for (message_path, _prev_message) in &prev_messages {
            if !curr_messages.contains_key(message_path) {
                changes.push(create_breaking_change(
                    ...
                    format!(
                        "Message \"{}\" was deleted from package \"{}\".",
                        message_path, prev_package
                    ),
        ...
        for (service_name, _prev_service) in &prev_services {
            if !curr_services.contains_key(service_name) {
                changes.push(create_breaking_change(
                    ...
                    format!(
                        "Service \"{}\" was deleted from package \"{}\".",
                        service_name, prev_package
                    ),
                    create_location(&context.current_file, "package", prev_package),
                    Some(create_location(
                        context.previous_file.as_deref().unwrap_or(""),
...
pub fn check_package_extension_no_delete(
    ...
    if prev_package == curr_package && !prev_package.is_empty() {
        ...
        let prev_extensions: HashMap<String, &crate::canonical::CanonicalExtension> = previous.extensions.iter()
            .map(|ext| (format!("{}.{}", ext.extendee, ext.number), ext))
            .collect();
        let curr_extensions: HashMap<String, &crate::canonical::CanonicalExtension> = current.extensions.iter()
            .map(|ext| (format!("{}.{}", ext.extendee, ext.number), ext))
            .collect();
...
pub const PACKAGE_RULES: &[(&str, fn(&CanonicalFile, &CanonicalFile, &RuleContext) -> RuleResult)] = &[
    ...
    ("PACKAGE_EXTENSION_NO_DELETE", check_package_extension_no_delete),
];